...
function [PosSnew] = moveSwimmer2(PosSold1,DuDiS1,Ns1,FS1,dt1,DSr1,FTSi1,VS1,SWT1,Ysi1,Xsi1,k1,RS1,moMax1,kickType1,SDV1,SBT1,polarity1,kickMethod1,wTorq01,stream)
    ...
    for i = 1:Ns1
        ...
        oriIndic = -PosSold1(i,9)*sign(cos(PosSold1(i,3)));
        ...
        PosSnew(i,6) = dt1*dx + dt1*VS1*c;
        PosSnew(i,2) = PosSold1(i,2) + dt1*dy + dt1*VS1*s; % New Y pos.
        PosSnew(i,7) = dt1*dy + dt1*VS1*s;
        ...
        if kickMethod1 == 1
            ...
            event = max(abs(PosSold1(i,9)),sign(p - rand(stream)));
            ...
            PosSnew(i,8) = event*(s1 + s2); % get the signed outcoome, multiply by the event check.
