...
function [DuDiC1] = dUdiColloidLJ(posSold1,posCold1,SDV1,RS1,RC1,Ns1,Nc1,eps1,wall1)
    ...
    % set the c/w interaction lengths as sum of their radii.
    sigCW = RC1/2^(1/6);
...
%         % Get all colloids excluding c1.
%         c2R = find(1:Nc1~=c1);
%         
%         % Colloid-colloid loop.
%         for s = c2R % Int. from colloid s.
% 
%                 % Get coords of seg. of 1: r1, and seg. of 2: r2.
%                 r1 = [posCold1(c1,1), posCold1(c1,2)];
...
%                 dr = sqrt((r1(:,1) - r2(:,1))^2 + (r1(:,2) - r2(:,2))^2);
%                 dx = abs(r1(:,1) - r2(:,1))*rm1;
%                 dy = abs(r1(:,2) - r2(:,2))*rm1;
%                 drm1 = 1/dr;
% 
%                 % get the interaction length and its inverse.
%                 l = lsc(seg1);
%                 lm1 = 1/l;
% 
%                 % Get the potential (simplifies calculation of gradients).
%                 u = u01*l*drm1*exp(-dr*lm1);
% 
%                 % Get the gradients.
%                 DuDiC1(c1,1) = DuDiC1(c1,1) + l*dx*drm1*(rdm1+lm1)*u;
%                 DuDiC1(c1,2) = DuDiC1(c1,2) + l*dy*drm1*(rdm1+lm1)*u;
% 
%         end
